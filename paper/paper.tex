...
disclosure policies. Sigil is a multichain provenance layer with a pluggable
\emph{confidential execution layer}: applications running on Ethereum, Solana,
Cardano, or other ecosystems generate verifiable provenance through Sigil, while
an interchangeable confidential backend handles commitments, selective
disclosure, and zero-knowledge verification.  We realize this layer on the
Aztec Network -- a privacy-first Ethereum layer-2 whose contracts are written in
Noir and whose private functions execute client-side -- and specify Midnight as
a second backend, so that no single ecosystem is load-bearing for the
design. We present the system architecture, formal graph model,
...
provenance that roots its confidentiality in a zero-knowledge backend while
remaining portable across both execution substrates and confidential backends.
\end{abstract}
...
innovations.  At the center of these innovations is a pluggable
\emph{confidential execution layer}: the zero-knowledge root in which every
commitment is anchored and against which every selective disclosure is
verified, so that provenance generated on any execution substrate can be
confidentially verified everywhere.  Crucially, the network occupying that
layer is a replaceable component rather than a premise of the design.  First, it
introduces a \emph{backend-agnostic provenance engine}
...
attributes.  Fourth, it separates the \emph{provenance model} from the
\emph{confidential backend} that enforces it: Sigil owns the trust graph,
attestation schema, and disclosure logic, while commitment anchoring and
zero-knowledge proof generation are delegated to an interchangeable
confidential network beneath the same five-method interface.  We realize this
on the \emph{Aztec Network}~\cite{aztec2023}, whose Noir~\cite{noir2023}
contracts execute privately on the client and settle to Ethereum, and specify
Midnight~\cite{midnight2023} as a second backend, so that applications
executing on other chains -- Ethereum, Solana, Cardano, and beyond -- obtain
confidential provenance through Sigil without revealing underlying data, and so
that the viability of the architecture is not tied to the fortunes of any one
network.  Fifth, it
...
representing provenance on the Web~\cite{w3c-prov2013}. PROV defines three core
...
others. The PROV-O ontology~\cite{groth2013} maps this model to RDF/OWL,
...
Public blockchains offer an attractive substrate for provenance: since
Bitcoin~\cite{nakamoto2008} they provide append-only immutable ledgers with
global verifiability.  Programmable smart-contract platforms such as
Ethereum~\cite{buterin2014,wood2014}, building on early formalizations of
digital contracts~\cite{szabo1997}, extended this substrate with general
computation.  However, the transparency that enables verification also creates
a fundamental tension with confidentiality~\cite{xu2017,eberhardt2017}.  Every
transaction, every attribute, every relationship recorded on a public chain is
visible to all participants.  For supply chain provenance, this means
competitors can observe supplier relationships, pricing structures, and
production volumes -- and, for proof-of-work networks, verification carries
substantial energy costs~\cite{sedlmeir2021}.

Layer-two protocols~\cite{gudgeon2020} and sidechains offer some scalability
relief, and hybrid on-chain/off-chain designs improve data retention and
storage efficiency~\cite{renner2023}, but neither addresses the confidentiality
problem.  Private blockchains
...
Privacy-preserving cryptography has a long lineage, from blind
signatures~\cite{chaum1982} to the privacy-enhancing technologies now pervasive
in decentralized systems~\cite{meiklejohn2024}.  Zero-knowledge proofs (ZKPs)
enable a prover to convince a verifier of a
...
The Aztec Network~\cite{aztec2023} is a privacy-first Ethereum layer-2 built
for confidential smart contracts.  A single Aztec contract may declare both
private and public state: private state is held as encrypted, UTXO-style notes,
while public state is executed by a public virtual machine.  Contracts are
written in Aztec.nr, a framework over the \emph{Noir}~\cite{noir2023}
zero-knowledge domain-specific language, and proved with a
PLONK-family~\cite{gabizon2019} proving system.  The property most consequential
for provenance is \emph{where} proving happens: private functions execute
client-side in the Private Execution Environment (PXE), so private inputs never
leave the holder's machine, and no third-party prover service is interposed
between a data holder and a proof about that data.  Aztec settles to Ethereum,
which additionally places Sigil's disclosures within reach of the identity,
attestation, and wallet standards already established on that
chain~\cite{buterin2014}.  These properties -- private/public state separation,
native selective disclosure, client-side proving, and L1 settlement -- make
Aztec the confidential backend Sigil realizes first.

A comparable design is the Midnight Network~\cite{midnight2023}, developed by
Input Output Global, which introduces a data-protection-first blockchain
architecture based on the Kachina protocol.  Midnight likewise combines a public
ledger for unshielded state with zero-knowledge circuits for confidential
computation; contracts written in the Compact language declare both public and
private state, and the runtime automatically generates ZK proofs for
transitions involving private data.  Its selective-disclosure model maps onto
Sigil's policy semantics as directly as Aztec's does, and we therefore specify
it as a second confidential backend.  Its adoption is deferred rather than
rejected: the developer ecosystem, wallets, and identity and attestation
integrations that a provenance layer must interoperate with are, at present,
substantially more established around Ethereum.  Retaining a fully specified
alternative is deliberate — it is what distinguishes a genuinely pluggable
confidential layer from a single anchor with an abstraction placed in front of
it.
...
architecture is designed to accommodate ZKP-based backends (such as Aztec and
Midnight) and TEE-based backends alike through its pluggable interface.
...
Privacy-preserving data sharing has been explored in adjacent industrial IoT
settings through blockchain and federated learning~\cite{lu2021}, but without a
general, backend-agnostic provenance abstraction.

\subsection{Software Supply Chain Security}

Motivated by the rise of software supply chain attacks on open-source package
managers~\cite{duan2021}, the software supply chain security community has
developed several frameworks
...
% Layer 5: Confidential Execution
\node[box, fill=teal!60, below=of l4] (l5)
  {\textbf{Confidential Execution}\qquad Aztec (active) \textbar{} Midnight (deferred)};

% Layer 6: Settlement
\node[box, fill=teal!45, below=of l5] (l6)
  {\textbf{Settlement}\qquad Ethereum (state and proof finality)};

% Layer 7: Execution / Interoperability
\node[box, fill=teal!35, below=of l6] (l7)
  {\textbf{Execution / Interoperability}\qquad Ethereum \textbar{} Solana \textbar{} Cardano \textbar{} Avalanche \textbar{} Local};
...
\draw[arrow] (l5.south) -- (l6.north) node[midway,right,font=\tiny] {settlement};
\draw[arrow] (l6.south) -- (l7.north) node[midway,right,font=\tiny] {data flow};

\end{tikzpicture}
\caption{System Architecture. Sigil's seven-layer design separates application
  logic, developer APIs, provenance computation, backend abstraction, the
  confidential execution layer, settlement, and the execution/interoperability
  layer where applications run.  Provenance ultimately commits through whichever
  confidential backend is active, regardless of where the application executes;
  that backend is interchangeable.  Ethereum appears twice in distinct roles --
  as settlement beneath the confidential backend, and as one execution substrate
  among several.  Data flow arrows indicate the request-response communication
  pattern between adjacent layers.}
\label{fig:layers}
\end{figure*}
...
\textbf{Confidential Execution Layer} is the confidential root of the system:
the active backend anchors commitments, executes selective-disclosure circuits,
and generates and verifies the zero-knowledge proofs on which Sigil's
confidentiality guarantees depend.  Aztec occupies this layer today and Midnight
is specified as an alternative; because both sit behind the same five-method
interface, replacing one is an adapter change rather than a redesign.  The
\textbf{Settlement Layer} is where the confidential backend's state and proofs
achieve finality -- Ethereum, in the case of Aztec -- giving disclosures an
L1-verifiable path. The \textbf{Execution / Interoperability
Layer} is where applications actually run -- Ethereum, Solana, Cardano,
Avalanche, a local SQLite adapter for development, and others -- each committing
its provenance \emph{through} the confidential backend via Sigil rather than
serving as an independent root of trust. An application native to any chain thus
obtains confidential provenance without that chain needing confidential
...
anchoring claims on a confidential backend, verifying signatures against Ethereum,
...
\begin{figure}[t]
\centering
% The plugin ring is wider than a single IEEEtran column; scale it to fit so it
% does not overflow into neighbouring text or figures.
...
Sigil delegates zero-knowledge selective disclosure to its confidential
backend -- Noir circuits proved client-side on Aztec, or Kachina-based Compact
circuits on Midnight.  When a viewer requests access to a subset of a claim's
...
\subsection{Integration with the Confidential Backend}

In the confidential backend, policies are compiled into contract circuits --
Noir circuits in an Aztec.nr contract on Aztec, or Compact circuits on Midnight.
...
The backend runtime then generates a single ZK proof attesting that all
...
policy prescribed -- no more, no less.  Because the constraint mapping is
identical under either circuit language, a disclosure policy is portable across
confidential backends; on Aztec, proving additionally occurs client-side in the
PXE, so the private witnesses -- the undisclosed fields -- never leave the
holder's machine.
...
\begin{itemize}
  ...
  \item \texttt{@sigil/backend-aztec} (planned): Aztec confidential-backend
        adapter, integrating \texttt{aztec.js} for contract deployment, circuit
        calls, and client-side ZK proof generation, with \texttt{noir} holding
        the corresponding Aztec.nr circuits.  A parallel
        \texttt{@sigil/backend-midnight} adapter is specified but deferred.
        Neither confidential backend is implemented in the current release; the
        packages listed above constitute what has been built.
...
Full disclosure (Aztec)$^{\dagger}$ & 840       & ms \\
\bottomrule
\end{tabular}

\vspace{2pt}
{\footnotesize $^{\dagger}$Projected, not measured: no confidential backend is
implemented in the current release.  All other rows are measured against the
local backend.}
\end{table}
...
The full disclosure figure is a projection rather than a measurement: it is
the budget we design the confidential backend against, and it is reported here
to make that target explicit, not to claim a result.  The operation encompasses
the end-to-end ZK proof generation cycle: serializing the claim, executing the
disclosure circuit, generating the proof, and verifying it.  On Aztec this cycle
runs client-side in the PXE, so the projected latency is dominated by proving
time rather than by round-trips to an external prover service -- a structural
difference from a remote-prover design, which trades a network dependency for
local compute.  Validating this figure against a working adapter is immediate
future work.
...
truthful (Sigil verifies signatures, not facts), that the confidential backend
operates correctly (its liveness depends on that network's consensus protocol
and, for Aztec, on Ethereum settlement), that
...
audited.  Two design choices bound the dependence this creates.  Client-side
proving in the PXE removes the third-party prover from the trust boundary
entirely: private witnesses are never disclosed to an external proving service.
And because the confidential layer is pluggable, a second backend is already
specified rather than hypothetical, so dependence on any single network is a
migration cost rather than a redesign.
...
on a confidential backend for ZK proofs, and none is implemented yet; until one
ships, selective disclosure is policy-gated but not cryptographically enforced.
The pluggable design bounds but does not eliminate this: a deployment that can
reach no confidential backend loses the selective-disclosure capability,
whichever networks are specified.  Third, the TypeScript implementation,
...
inclusion proofs, and pluggable confidential backend -- through which applications
on any execution chain anchor their provenance -- together form a
...
via the confidential backend.  These benchmarks confirm that confidential provenance at scale
...
\bibitem{aztec2023}
Aztec Network, ``Aztec: A Privacy-First Layer-2 Network on Ethereum,''
\textit{Aztec Labs Technical Documentation}, 2023. [Online]. Available:
\url{https://aztec.network}

\bibitem{noir2023}
Aztec Labs, ``Noir: A Domain-Specific Language for Zero-Knowledge Proofs,''
...
\textit{Cryptology ePrint Archive}, Report 2019/953, 2019.
