...
disclosure policies. Sigil is a multichain provenance layer anchored by the
Midnight Network: applications running on Ethereum, Solana, Cardano, or other
ecosystems generate verifiable provenance through Sigil, while Midnight serves
as the confidential trust anchor for commitments, selective disclosure, and
zero-knowledge verification. We present the system architecture, formal graph model,
...
provenance that roots its confidentiality in a zero-knowledge trust anchor
while remaining portable across execution backends.
\end{abstract}
...
innovations.  At the center of these innovations is the \emph{Midnight
Network}, which Sigil adopts as its confidential \emph{trust anchor}: the
zero-knowledge root in which every commitment is anchored and against which
every selective disclosure is verified, so that provenance generated on any
execution substrate can be confidentially verified everywhere.  First, it
...
attributes.  Fourth, it establishes the \emph{Midnight Network} as a
confidential \emph{trust anchor} rather than one backend among many: Sigil roots
its confidentiality in Midnight, anchoring commitments and generating
zero-knowledge selective-disclosure proofs via Midnight's Kachina protocol, so
that applications executing on other chains -- Ethereum, Solana, Cardano, and
beyond -- obtain Midnight-level confidential provenance through Sigil without
revealing underlying data~\cite{midnight2023}.  Fifth, it
...
observers.  These properties -- public/private state separation, native
selective disclosure, and automatic Kachina-based proof generation -- make
Midnight uniquely suited to serve as Sigil's confidential \emph{trust anchor},
the role it occupies throughout this work: the root in which Sigil anchors its
commitments and against which every zero-knowledge disclosure is verified,
independent of the chain on which an application executes.
...
\node[box, fill=teal!60, below=of l4] (l5)
  {\textbf{Trust Anchor Layer}\qquad Midnight Network (confidential ZK anchor)};

% Layer 6: Execution / Interoperability
\node[box, fill=teal!35, below=of l5] (l6)
...
\caption{System Architecture. Sigil's six-layer design separates application
  ...
  Midnight trust anchor, and the execution/interoperability layer where
  applications run.  Everything ultimately anchors into Midnight regardless of
  where the application executes.  Bidirectional data flow arrows indicate the
  request-response communication pattern between adjacent layers.}
\label{fig:layers}
\end{figure}
...
\textbf{Trust Anchor Layer} is the confidential root of the system: the Midnight
Network anchors commitments, executes selective-disclosure circuits, and
generates and verifies the zero-knowledge proofs on which Sigil's
confidentiality guarantees depend. The \textbf{Execution / Interoperability
Layer} is where applications actually run -- Ethereum, Solana, Cardano,
Avalanche, a local SQLite adapter for development, and others -- each anchoring
its provenance \emph{into} Midnight through Sigil rather than serving as an
independent root of trust. An application native to any chain thus obtains
Midnight-level confidential provenance without that chain needing confidential
...
Sigil's Midnight trust anchor leverages the Kachina protocol for zero-knowledge
...
At the Midnight trust anchor, policies are compiled into Compact contract circuits.
...
\begin{itemize}
  ...
  \item \texttt{@sigil/backend-midnight}: Midnight Network trust-anchor adapter
        integrating the Midnight.js SDK for contract deployment, circuit calls,
...
The full disclosure operation at the Midnight trust anchor encompasses the
...
audited.  Midnight serves as the system's confidential trust anchor by design;
future work will explore self-hosted prover options and multi-prover
architectures to reduce dependence on a single anchor.
...
on the Midnight Network as its trust anchor for ZK proofs; users who cannot
access Midnight lose the selective disclosure capability.  Third, the TypeScript implementation,
...
inclusion proofs, and Midnight Network trust anchor -- into which applications
